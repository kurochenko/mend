...
Mend authenticates an incoming provider webhook, validates the supported payload, and converts it into the shared review or note event consumed by the queue and conversation handlers. The [[review.con:review-provider]] boundary supplies provider-specific identity and thread lookup only after self-authored note events have been discarded.
